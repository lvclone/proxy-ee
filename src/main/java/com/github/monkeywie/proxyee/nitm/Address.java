...
import com.github.monkeywie.proxyee.nitm.exception.ProxyeeProxyException;
    ...
    public static Address resolve(String address) {
        ...
        if (matcher.find()) {
            int port = Optional.ofNullable(matcher.group(2))
                               ...
                               .orElseThrow(() -> new ProxyeeProxyException("Invalid address: " + address));
            return new Address(matcher.group(1), port);
        } else {
            throw new ProxyeeProxyException("Invalid address: " + address);
        }
    }
